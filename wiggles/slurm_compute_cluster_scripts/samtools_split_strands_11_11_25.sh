...
# splits BAM files by strand
# sbatch samtools_split_strands_11_11_25.sh <paired/unpaired> <forward/reversed> </path/to/bamfiles/>
if [ ! -d strands ]; then
	mkdir strands
fi
...
if [ $paired_end == "unpaired" ]; then
	...
	if [ $reversed_seq == "forward" ]; then
		...
		for file in $samfile_path/*.bam; do
			...
			sbatch -n 24 --wrap="samtools view -b -q 30 -f 16 $file > strands/"$file_name"_-_if_forward_stranded_f16.bam"

			# Next we map to the "forward" strand, excluding the 16 flag
			sbatch -n 24 --wrap="bamtools view -b -q 30 -F 16 $file > strands/"$file_name"_+_if_forward_stranded_F16.bam"
		done
		...
		for file in $bamfile_path/*.bam; do
		...
		echo "Please use the following format to run this script: bash samtools_split_strands_1_30_24.sh <paired/unpaired> <forward/reversed> </path/to/bamfiles>"
		exit
	...
	if [ $reversed_seq == "forward" ]; then
		...
		for file in $bamfile_path/*.bam; do
			...
			sbatch -n 24 --wrap="samtools view -b -q 30 -f 83 $file > strands/"$file_name"_-_83.bam
			samtools view -b -q 30 -f 163 $file > strands/"$file_name"_-_163.bam
			...
			sbatch -n 24 --wrap="samtools view -b -q 30 -f 99 $file > strands/"$file_name"_+_99.bam
			samtools view -b -q 30 -f 147 $file > strands/"$file_name"_+_147.bam
		...
		echo "Please use the following format to run this script: bash samtools_split_strands_11_11_25.sh <paired/unpaired> <forward/reversed> </path/to/bamfiles>"
		exit
	fi
