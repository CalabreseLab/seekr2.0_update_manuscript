...
def normalize_wiggles(exp_dict, control_dict):
    ...
    for chrom in exp_dict:
        ...
        for position in exp_dict[chrom]:
            ...
            else: # if chrom is not in the control dict at all
                adj_dict[chrom][position] = float(exp_dict[chrom][position]) # no adjustment to signal needed
    return(adj_dict)
...
if __name__ == "__main__":
    ...
    outfile_name = exp_file.replace(".wig", "")
