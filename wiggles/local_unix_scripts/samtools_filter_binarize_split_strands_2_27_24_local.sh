...
# filters data with -q 30, binarizes to a bam file, and splits by strand
...
if [ $paired_end == "unpaired" ]; then
	...
	if [ $reversed_seq == "forward" ]; then
		...
		for file in $samfile_path/*.sam; do
			...
			samtools view -b -q 30 -f 16 $file > strands/"$file_name"_-_if_forward_stranded_f16.bam
		...
		for file in $samfile_path/*.sam; do
			...
			file_name=$(basename $file .sam)
                        ...
                        samtools view -b -q 30 -f 16 $file > strands/"$file_name"_+_if_reverse_stranded_f16.bam
	...
	if [ $reversed_seq == "forward" ]; then
		...
		for file in $samfile_path/*.sam; do
			...
			samtools view -b -q 30 -f 83 $file > strands/"$file_name"_-_83.bam
			...
			samtools view -b -q 30 -f 99 $file > strands/"$file_name"_+_99.bam
